...
**Fix.** `whenNotPaused` now gates every verification entry point: `verifyAssetCompliance`, `areTokensCompliant`, `verifyRouterAuthorization`, `verifyExecutorStatus`, and the `checkSubscriptionCompliance` gate. When paused they revert `EnforcedPause()` (fail-closed) rather than returning a value, so the pause propagates to every integrator (TeleportV2/V3 and external callers such as an integrating fund-subscription manager). `TeleportV3.onlyAuthorized` was reordered so the protocol owner short-circuits before the (now pause-gated) registry call and cannot be locked out of the authorization gate by a registry pause, while the in-body asset-compliance check still enforces the halt.
